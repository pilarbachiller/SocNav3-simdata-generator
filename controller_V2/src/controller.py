...
if __name__ == '__main__':
    ...
    # Create a proxy to publish a SNGNN2D topic
    topic = False
    try:
        topic = topicManager.retrieve("SNGNN2D")
    except:
        pass
    while not topic:
        try:
            ...
            try:
                topic = topicManager.create("SNGNN2D")
            except:
                print('Another client created the SNGNN2D topic? ...')

    pub = topic.getPublisher().ice_oneway()
    sngnn2dTopic = SNGNN2DPrx.uncheckedCast(pub)
    mprx["SNGNN2DPub"] = sngnn2dTopic
