...
int ::trajectory::run(int argc, char* argv[])
{
	...
	{
		cout << "Problem reading FrameOfReference property\n";
	}

	string DrawObs;
	if (not GenericMonitor::configGetString(communicator(), prefix, "DrawObjects", DrawObs, ""))
	{
		cout << "Problem reading FrameOfReference property\n";
	}
	...
	bool WorldFR = false;	
	if(FR.compare("World")==0)
		WorldFR = true;

	bool DrawObstacles = true;
	if(DrawObs.compare("False")==0)
		DrawObstacles = false;
	...
	SpecificWorker *worker = new SpecificWorker(mprx, WorldFR, DrawObstacles);
	//Monitor thread
