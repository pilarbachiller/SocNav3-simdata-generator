...
// #define SOCNAV_AREA_WIDTH 1330.
#define GRID_SIZE1 100
...
Q_OBJECT
public:
    SpecificWorker(MapPrx& mprx, bool WorldFR, bool DrawObstacles);
    ~SpecificWorker();
...
private:
    ...
    bool WorldFR, DrawObstacles;

    float SOCNAV_AREA_WIDTH;
    ...
    void find_free_target(int gx, int gy, int & new_gx, int & new_gy, int w);
    std::vector<Point> find_free_path(Mat grid, Point start, Point goal);
    bool check_free_path(std::vector<Point> path, Mat grid);
    void find_linear_new_target(std::vector<Point> &path, Mat grid);
    ...
    float compute_min_area_cost(Mat& _grid, Point center, int w);
